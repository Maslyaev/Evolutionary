...
class Population:
    def __init__(self, evaluator, eval_params, tokens, token_params, pop_size, a_proc,
                 r_crossover, r_param_mutation, r_mutation, mut_chance, alpha, eq_len = 8, max_factors_in_terms = 2, max_power = 2): 
...
#        print('in population: tokens:', type(tokens), tokens)
        ...
        self.population = [Equation(self.tokens, self.token_params, evaluator, eval_params, eq_len, max_factors_in_terms) for i in range(pop_size)]
...
#                print(self.population[0].terms[idx].gene, self.population[0].weights[idx-1])
    ...
    def Initiate_Evolution(self, iter_number, estimator_type, log_file = None, test_indicators = False):
        ...
        for idx in range(iter_number):
            ...
            strict_restrictions = False if idx < iter_number - 1 else strict_restrictions = True
...
def Get_true_coeffs(evaluator, eval_params, tokens, equation, max_power = 2):
    ...
    target_vals = target.Evaluate_term(evaluator, eval_params)    
...
#            feature_vals = np.reshape(feature_vals, np.prod(feature_vals.shape))
            ...
            features_list.append(equation.terms[i].Evaluate_term(evaluator, eval_params) )
