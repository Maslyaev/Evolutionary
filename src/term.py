...
class Term:
    def __init__(self, gene = None, tokens_list = ['1', 'u'], token_params = collections.OrderedDict([('power', (0, 4))]),
    ...
    def Randomize_Gene(self): # Разобраться с коидровкой
        self.label_dict = {}
        ...
        for factor_idx in range(factor_num):
            while True:
                key = np.random.choice(list(self.label_dict.keys()))
        ...
        for token in list(self.label_dict.keys()):
            for param in self.label_dict[token].keys():
    ...
    def Remove_Dublicated_Factors(self, allowed_tokens, background_terms): # Переписать
        ...
        for idx in range(allowed_tokens.size):      # Filtration of forbidden tokens (that are present in the target for regression) from term 
            if self.gene[idx*self.n_params + list(self.token_params.keys()).index('power')] > 0 and not allowed_tokens[idx]:
                factors_cleared += self.gene[idx*self.n_params + list(self.token_params.keys()).index('power')]
...
#            allowed.remove(0)
        ...
        while True:
            ...
            for i in range(factors_cleared):
                ...
                gene_cleared[idx*self.n_params + list(self.token_params.keys()).index('power')] += 1
                if gene_cleared[idx*self.n_params + list(self.token_params.keys()).index('power')] == self.token_params['power'][1]:
